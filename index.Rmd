...
Create package folder and files `r emo::ji("file")`
...
* There are some hidden files added too; we don't need to worry about these for now
---
class: middle
...
Generate documentation `r emo::ji("document")`
...
* each function by adding roxygen
---
class: middle
...
Add roxygen above functions to document them
...
The roxygen was converted to `.Rd`
...
cabinet_cat <- function(name) {
  ...
  if (name %in% c("Ossie", "Evie")) {
    cat(glue::glue("{name} is a good Cabinet Office kitty.")) #<<
  } else if (name %in% c("Palmerston", "Larry", "Gladstone")) {
    cat(glue:glue("{name} isn't a Cabinet Office cat!")) #<<
  } else {
    cat("You've much to learn about government cats.")
  }
...
Declare that {glue} is needed on install
...
{glue} added to 'Imports' field in `DESCRIPTION`
...
```
???
* {glue} wil be installed when {cabinet} is installed
...
* You can use 'Suggests' to suggest, but not install, packages
...
#   Name:        cabinet
#   Description: A Silly Package To Demonstrate How To Build Packages
...
Add a skeleton README to give an overview of the package `r emo::ji("memo")`
...
Add a vignette to explain how to do something
...
Install {pkgdown} to create a website for your package
...
Make it live in the repo settings
